    ...
    def run(self, plotting=True):
        if plotting:
            ...
            def init():
                ...
                # Initially set ylim based on initial data range
                ax.set_ylim(min(self.data["price"]) - 2, max(self.data["price"]) + 2)
                return (
                    line_price,
                    line_upper,
                    line_lower,
            ...
            def update(frame):
                ...
                # Dynamically adjust y-limits based on latest data
                if self.upper_bands[-1] is not None and self.lower_bands[-1] is not None:
                    # Use the last upper and lower bands to set the y-limits
                    ylim_lower = self.lower_bands[-1] - 2  # Subtract margin
                    ylim_upper = self.upper_bands[-1] + 2  # Add margin
                else:
                    # If bands are not available yet, use the min and max of the current price window
                    current_window = self.price_history[-self.strategy.boll_window :]
                    ylim_lower = min(current_window) - 2
                    ylim_upper = max(current_window) + 2

                ax.set_ylim(ylim_lower, ylim_upper)

                ax.set_xlim(max(0, len(self.price_history) - self.strategy.boll_window), len(self.price_history) + 10)
...
# data = pd.read_csv("your_data.csv", names=["time", "price", "volume", "symbol"])
